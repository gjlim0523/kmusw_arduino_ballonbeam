...
void loop() {
  ...
  if (dist_raw < _TARGET_LOW || dist_raw = _TARGET_LOW) {
    myservo.writeMicroseconds(_DUTY_MIN);
  }
  else if (dist_raw > _TARGET_HIGH || dist_raw = _TARGET_HIGH) {
    myservo.writeMicroseconds(_DUTY_MAX);
  }
  else {
    myservo.writeMicroseconds(_DUTY_MIN + (dist_raw - 180) * 923 / 90);
  }
